...
- **develop** `45274e8` — staging (`staging.fourgo.it`)

## Novità sessione 22 luglio (giornata lunga, molti fix reali da segnalazioni utente)
- **Bug critico social-post → SocialPost**: `pkg` dichiarata dentro un blocco try{} andava fuori scope prima del codice di salvataggio — errore reale a runtime, inghiottito silenziosamente dal catch{} vuoto. Zero righe salvate nonostante pubblicazione riuscita su tutte le piattaforme. Confermato risolto con test reale (post Uzbekistan salvato correttamente in tutte e 4 le piattaforme)
- **Annuncio di piani futuri attivava l'audioguida**: "domani andiamo al palazzo di X" faceva scattare narrazione completa non richiesta — `placeKeywords` troppo permissiva (parole generiche tipo "palazzo", "chiesa", "museo"). Aggiunta categoria PLAN al classificatore AI esistente, un annuncio di piani non attiva più l'audioguida
- **Domande generiche su un luogo non attivavano mai la ricerca esterna**: "cosa mi dici della spiaggia di Marathi" → Violetta ammetteva onestamente di non sapere ma senza mai provare a cercare. Aggiunta prima `isPlaceInfoQuery` (parole chiave), poi — su suggerimento — una **rete di sicurezza con Haiku** (`needsExternalSearchAI`) per le formulazioni future non previste dalle liste fisse, chiamata solo se nessuna parola chiave scatta (nessuna latenza aggiunta al caso comune)
- **3 stemmi comunali rotti** (Solaro, Meda, Muggiò) nella pagina città — nomi file Wikimedia sbagliati (mancava il disambiguante "(Italia)" per i primi due, accento sbagliato ó/ò per Muggiò). Verificato con script diagnostico + ricerca diretta dei nomi file veri, non per tentativi. **Nota per il futuro**: Wikimedia applica rate limiting aggressivo (429) a richieste multiple ravvicinate dallo stesso IP — uno script che controlla molti file in sequenza va rallentato (pausa 500ms+ tra richieste) o rischia falsi positivi
- **TikTok Developer abbandonato definitivamente**: review rifiutata per policy (non un problema di formulazione — TikTok non approva app di singola azienda che pubblica sul proprio account, solo strumenti multi-tenant). Pubblicazione resta manuale (ora un po' più snella: uniti i passaggi carica-video+genera-caption in un solo bottone, aggiunto bottone dedicato copia-caption-per-TikTok), dettagli completi in "Sessione 19 luglio" più sotto
