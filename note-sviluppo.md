...
### 5 settembre 2026 — pattern trovati costruendo il matching Adamantis (4GO-26)

**Un totale sommato per tappa non tollera un prezzo unico frazionato:** se un fornitore terzo dà un prezzo unico per un intero pacchetto multi-tappa, e il codice esistente somma il prezzo di ogni tappa per il totale (`legs.reduce(...) + hotel.totalPrice`), mettere lo stesso prezzo su più tappe moltiplica il totale mostrato al cliente. Verificare sempre come viene aggregato un campo prezzo PRIMA di decidere come strutturare dati importati da fuori, non dopo.

**Un "budget dichiarato" è quasi sempre un tetto, non un bersaglio:** minimizzare `|prezzo - budget|` in entrambe le direzioni fa scegliere un'opzione sopra budget se numericamente più vicina di una ampiamente sotto — sbagliato quando il cliente scrive "fino a X". Tra le opzioni entro il tetto, scegliere la più cara (miglior valore permesso), non la più vicina in assoluto.

**Un indice "leggero" non garantisce che ogni riga abbia lo stesso tipo di dato:** pacchetti della stessa fonte ma categorie diverse (qui: "Tour" vs "Pacchetto Land" di Adamantis) possono avere disponibilità di dati strutturalmente diversa (prezzo presente solo in una delle due). Un filtro che sceglie candidati per un solo criterio (qui: vicinanza durata) rischia di riempire la selezione con candidati tutti privi del secondo dato che serve (prezzo) su destinazioni sbilanciate verso la categoria senza quel dato — filtrare esplicitamente per categoria quando quel dato è necessario, non solo confidare nel campione.
